...
public class IbanServiceImpl implements IbanService {
    ...
    public boolean isIbanValid(String code) {
        ...
        if (ibanLengthMap.ibanLengthValidation(code)){
            ...
            isValid = modulOperationIBAN(ibanUtils.ibanNumberCode(code));
        } else
            isValid = false;
        return isValid;
    }
    ...
    private boolean modulOperationIBAN(String iban) {
        ...
        while (j < iban.length() + 8) {
            if (i == 0) {
                N = Integer.parseInt(ibanUtils.ibanNumberCode(iban).substring(i, j));
                N = N % 97;
            } else if (j < iban.length()) {
                N = Integer.parseInt(N + "" + ibanUtils.ibanNumberCode(iban).substring(i, j));
                N = N % 97;
            } else {
                N = Integer.parseInt(N + "" + ibanUtils.ibanNumberCode(iban).substring(i, iban.length()));
                N = N % 97;
            }
            if (N == 1) {
                isValid = true;
